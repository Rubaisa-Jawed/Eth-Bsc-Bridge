...
contract BridgeBase {
    ...
    function burn(address to, uint amount, uint nonce, bytes calldata signature) external {
        require(processedNonces[msg.sender][nonce] == false, 'Bridge Base: Transfer already processed');
    ...
    function mint(address from, address to, uint amount, uint nonce, bytes calldata signature) external {
        ...
        require(recoverSigner(message, signature) == from , 'Bridge Base: Wrong Signature');
        require(processedNonces[from][nonce] == false, 'Bridge Base: Transfer Already Processed');
